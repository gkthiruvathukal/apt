...
void ArgumentParse(OptCount, OptInfo, argc, argv, defaultAction)
int OptCount;
OptionDescriptorBlock OptInfo;
int argc;
char **argv;
OptionTriggerFunction defaultAction;
{
  char **argp,
  *MatchSet = NullString,
  **OptArgs = NULL,
  ...
  for (argp=argv + 1; argp < argv + argc; argp += offset) {
    ...
    if (MatchedOption == NULL) {
      ...
      if (MatchedOption->action)
        (*MatchedOption->action)(MatchedOption-OptInfo, Switch, MatchSet,
                                 ArgumentCount, OptArgs);
      /*
      else
        error();
      */
